...
function mergeSymbolMaps(additionalSymbols){var symbolMap={};symbolMap.symbols=additionalSymbols?_extends$5({},defaultSymbolMap.symbols,additionalSymbols.symbols):_extends$5({},defaultSymbolMap.symbols);return symbolMap;}function exec(operators,values){var _ref;var op=operators.pop();values.push(op.f.apply(op,(_ref=[]).concat.apply(_ref,values.splice(-op.argCount))));return op.precedence;}function calculate(expression,additionalSymbols){var symbolMap=mergeSymbolMaps(additionalSymbols);var match;var operators=[symbolMap.symbols['('].prefix];var values=[];var pattern=new RegExp(// Pattern for numbers
 ...
 * }
 */function darken(amount,color){if(color==='transparent')return color;var hslColor=parseToHsl(color);return toColorString(_extends$5({},hslColor,{lightness:guard(0,1,hslColor.lightness-parseFloat(amount))}));}// prettier-ignore
 ...
 * }
 */function lighten(amount,color){if(color==='transparent')return color;var hslColor=parseToHsl(color);return toColorString(_extends$5({},hslColor,{lightness:guard(0,1,hslColor.lightness+parseFloat(amount))}));}// prettier-ignore
 ...
 * Use of this source code is governed under the Apache License, Version 2.0
 * found at http://www.apache.org/licenses/LICENSE-2.0.
 */function ownerDocument$1(node){return node&&node.ownerDocument||document;}function activeElement$1(doc){if(doc===void 0){doc=ownerDocument$1();}try{var active=doc.activeElement;if(!active||!active.nodeName)return null;return active;}catch(e){return doc.body;}}const useFocusJail=function(_temp){let{focusOnMount=true,restoreFocus=true,environment,focusElem,containerRef}=_temp===void 0?{containerRef:React.createRef()}:_temp;const restoreFocusElement=React.useRef(null);const[currentRef,setCurrentRef]=React.useState(containerRef.current);React.useEffect(()=>{if(containerRef.current!==currentRef){setCurrentRef(containerRef.current);}});const focusElement=React.useCallback(element=>{if(focusElem){focusElem(element);}else{element&&element.focus();}},[focusElem]);const validateContainerRef=()=>{if(!currentRef){throw new Error('Accessibility Error: You must apply the ref prop to your containing element.');}};const getInitialFocusNode=()=>{const doc=environment?environment:document;const activeElem=activeElement$1(doc);const containerElem=currentRef;return containerElem.contains(activeElem)?activeElem:containerElem;};const getTabbableNodes=()=>{const elements=tabbable(currentRef);return{firstItem:elements[0]||getInitialFocusNode(),lastItem:elements[elements.length-1]||getInitialFocusNode()};};const getContainerProps=function(_temp2){let{onKeyDown,...other}=_temp2===void 0?{}:_temp2;return{onKeyDown:composeEventHandlers(onKeyDown,event=>{if(event.key!==KEYS.TAB){return;}validateContainerRef();const tabbableNodes=getTabbableNodes();if(event.shiftKey&&(event.target===tabbableNodes.firstItem||event.target===currentRef)){focusElement(tabbableNodes.lastItem);event.preventDefault();}if(!event.shiftKey&&event.target===tabbableNodes.lastItem){focusElement(tabbableNodes.firstItem);event.preventDefault();}}),'data-garden-container-id':'containers.focusjail','data-garden-container-version':'2.0.10',...other};};React.useEffect(()=>{const doc=environment||document;restoreFocusElement.current=activeElement$1(doc);if(focusOnMount){focusElement(currentRef);}return()=>{const isBodyInactive=restoreFocusElement.current!==doc.body;const hasActiveElement=restoreFocusElement.current!==null;if(isBodyInactive&&hasActiveElement&&restoreFocus){focusElement(restoreFocusElement.current);}};},[focusOnMount,restoreFocus,environment,focusElement,currentRef]);return{getContainerProps,focusElement};};({children:propTypes.exports.func,render:propTypes.exports.func,focusOnMount:propTypes.exports.bool,restoreFocus:propTypes.exports.bool,environment:propTypes.exports.any,containerRef:propTypes.exports.any.isRequired,focusElem:propTypes.exports.func});/**
 * Copyright Zendesk, Inc.
 *
 * Use of this source code is governed under the Apache License, Version 2.0
 * found at http://www.apache.org/licenses/LICENSE-2.0.
 */const useModal=_ref=>{let{onClose,modalRef,idPrefix,focusOnMount,restoreFocus,environment}=_ref;const prefix=useId(idPrefix);const titleId=`${prefix}__title`;const contentId=`${prefix}__content`;const isModalMousedDownRef=React.useRef(false);const closeModal=event=>{onClose&&onClose(event);};const getBackdropProps=function(_temp){let{onMouseUp,...other}=_temp===void 0?{}:_temp;const containerId='containers.modal';const handleMouseUp=event=>{const target=event.target;const isModalContainer=containerId===target.getAttribute('data-garden-container-id');if(!isModalMousedDownRef.current&&isModalContainer){closeModal(event);}isModalMousedDownRef.current=false;};return{onMouseUp:composeEventHandlers(onMouseUp,handleMouseUp),'data-garden-container-id':containerId,'data-garden-container-version':'1.0.10',...other};};const getModalProps=function(_temp2){let{role='dialog',onKeyDown,onMouseDown,...other}=_temp2===void 0?{}:_temp2;return{role:role===null?undefined:role,tabIndex:-1,'aria-modal':true,'aria-labelledby':titleId,'aria-describedby':contentId,onMouseDown:composeEventHandlers(onMouseDown,()=>{isModalMousedDownRef.current=true;}),onKeyDown:composeEventHandlers(onKeyDown,event=>{if(event.key===KEYS.ESCAPE){closeModal(event);}}),...other};};const getTitleProps=function(_temp3){let{id=titleId,...other}=_temp3===void 0?{}:_temp3;return{id,...other};};const getContentProps=function(_temp4){let{id=contentId,...other}=_temp4===void 0?{}:_temp4;return{id,...other};};const getCloseProps=_ref2=>{let{onClick,...other}=_ref2;return{onClick:composeEventHandlers(onClick,event=>{closeModal(event);}),...other};};const{getContainerProps}=useFocusJail({containerRef:modalRef,focusOnMount,restoreFocus,environment});return{getBackdropProps,getModalProps:props=>getContainerProps(getModalProps(props)),getTitleProps,getContentProps,getCloseProps,closeModal};};({children:propTypes.exports.func,render:propTypes.exports.func,onClose:propTypes.exports.func,modalRef:propTypes.exports.any.isRequired,idPrefix:propTypes.exports.string,focusOnMount:propTypes.exports.bool,restoreFocus:propTypes.exports.bool,environment:propTypes.exports.any});function mergeRefs(refs){return function(value){refs.forEach(function(ref){if(typeof ref==="function"){ref(value);}else if(ref!=null){ref.current=value;}});};}function isDocument(element){return'nodeType'in element&&element.nodeType===document.DOCUMENT_NODE;}function isWindow(node){if('window'in node&&node.window===node)return node;if(isDocument(node))return node.defaultView||false;return false;}/**
 ...
 */process.env.NODE_ENV!=='production';function _objectWithoutPropertiesLoose(source,excluded){if(source==null)return{};var target={};var sourceKeys=Object.keys(source);var key,i;for(i=0;i<sourceKeys.length;i++){key=sourceKeys[i];if(excluded.indexOf(key)>=0)continue;target[key]=source[key];}return target;}/**
...
if(phase==='active'){if(node)forceReflow(node);}if(className){this.appliedClasses[type][phase]=className;_addClass(node,className);}};_proto.removeClasses=function removeClasses(node,type){var _this$appliedClasses$=this.appliedClasses[type],baseClassName=_this$appliedClasses$.base,activeClassName=_this$appliedClasses$.active,doneClassName=_this$appliedClasses$.done;this.appliedClasses[type]={};if(baseClassName){removeClass(node,baseClassName);}if(activeClassName){removeClass(node,activeClassName);}if(doneClassName){removeClass(node,doneClassName);}};_proto.render=function render(){var _this$props=this.props;_this$props.classNames;var props=_objectWithoutPropertiesLoose(_this$props,["classNames"]);return/*#__PURE__*/React__default["default"].createElement(Transition,_extends$5({},props,{onEnter:this.onEnter,onEntered:this.onEntered,onEntering:this.onEntering,onExit:this.onExit,onExiting:this.onExiting,onExited:this.onExited}));};return CSSTransition;}(React__default["default"].Component);CSSTransition.defaultProps={classNames:''};CSSTransition.propTypes=process.env.NODE_ENV!=="production"?_extends$5({},Transition.propTypes,{/**
 ...
 */function activeElement(doc){if(doc===void 0){doc=ownerDocument();}// Support: IE 9 only
// IE9 throws an "Unspecified error" accessing document.activeElement from an <iframe>
try{var active=doc.activeElement;// IE11 returns a seemingly empty object in some cases when accessing
// document.activeElement from an <iframe>
if(!active||!active.nodeName)return null;return active;}catch(e){/* ie throws if no active element */return doc.body;}}/**
 * Copyright Zendesk, Inc.
 *
 * Use of this source code is governed under the Apache License, Version 2.0
 * found at http://www.apache.org/licenses/LICENSE-2.0.
 */function _extends$2$1(){_extends$2$1=Object.assign?Object.assign.bind():function(target){for(var i=1;i<arguments.length;i++){var source=arguments[i];for(var key in source){if(Object.prototype.hasOwnProperty.call(source,key)){target[key]=source[key];}}}return target;};return _extends$2$1.apply(this,arguments);}const COMPONENT_ID$j='modals.backdrop';const animationName$1=styled.keyframes(["0%{opacity:0;}100%{opacity:1;}"]);const animationStyles$1=props=>{if(props.isAnimated){return styled.css(["animation:"," 0.15s ease-in;"],animationName$1);}return'';};const StyledBackdrop=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$j,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledBackdrop",componentId:"sc-mzdjpo-0"})(["display:flex;position:fixed;inset:0;align-items:",";justify-content:",";z-index:400;background-color:",";overflow:auto;-webkit-overflow-scrolling:touch;font-family:",";direction:",";",";",";"],props=>props.isCentered&&'center',props=>props.isCentered&&'center',props=>getColor('neutralHue',800,props.theme,0.85),props=>props.theme.fonts.system,props=>props.theme.rtl&&'rtl',animationStyles$1,props=>retrieveComponentStyles(COMPONENT_ID$j,props));StyledBackdrop.defaultProps={theme:DEFAULT_THEME};StyledBackdrop.propTypes={isCentered:propTypes.exports.bool,isAnimated:propTypes.exports.bool};const COMPONENT_ID$i='modals.body';const StyledBody=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$i,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledBody",componentId:"sc-14rzecg-0"})(["display:block;margin:0;padding:",";height:100%;overflow:auto;line-height:",";color:",";font-size:",";",";"],props=>`${props.theme.space.base*5}px ${props.theme.space.base*10}px`,props=>getLineHeight(props.theme.lineHeights.md,props.theme.fontSizes.md),props=>props.theme.colors.foreground,props=>props.theme.fontSizes.md,props=>retrieveComponentStyles(COMPONENT_ID$i,props));StyledBody.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$h='modals.close';const colorStyles$1=props=>{const backgroundColor='primaryHue';const foregroundColor='neutralHue';return styled.css(["background-color:transparent;color:",";&:hover{background-color:",";color:",";}"," &:active{transition:background-color 0.1s ease-in-out,color 0.1s ease-in-out;background-color:",";color:",";}"],getColor(foregroundColor,600,props.theme),getColor(backgroundColor,600,props.theme,0.08),getColor(foregroundColor,700,props.theme),focusStyles({theme:props.theme,hue:backgroundColor}),getColor(backgroundColor,600,props.theme,0.2),getColor(foregroundColor,800,props.theme));};const BASE_MULTIPLIERS$1={top:2.5,side:6.5,size:10};const StyledClose=styled__default["default"].button.attrs({'data-garden-id':COMPONENT_ID$h,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledClose",componentId:"sc-iseudj-0"})(["display:block;position:absolute;top:","px;",":",";transition:box-shadow 0.1s ease-in-out,background-color 0.25s ease-in-out,color 0.25s ease-in-out;border:none;border-radius:50%;background-color:transparent;cursor:pointer;padding:0;width:","px;height:","px;overflow:hidden;text-decoration:none;font-size:0;user-select:none;&::-moz-focus-inner{border:0;}"," & > svg{vertical-align:middle;}",";"],props=>props.theme.space.base*BASE_MULTIPLIERS$1.top,props=>props.theme.rtl?'left':'right',props=>`${props.theme.space.base*BASE_MULTIPLIERS$1.side}px`,props=>props.theme.space.base*BASE_MULTIPLIERS$1.size,props=>props.theme.space.base*BASE_MULTIPLIERS$1.size,props=>colorStyles$1(props),props=>retrieveComponentStyles(COMPONENT_ID$h,props));StyledClose.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$g='modals.footer';const StyledFooter=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$g,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledFooter",componentId:"sc-d8pfdu-0"})(["display:flex;flex-shrink:0;align-items:center;justify-content:flex-end;border-top:",";padding:",";",";"],props=>props.isLarge&&`${props.theme.borders.sm} ${getColor('neutralHue',200,props.theme)}`,props=>props.isLarge?`${props.theme.space.base*8}px ${props.theme.space.base*10}px`:`${props.theme.space.base*5}px ${props.theme.space.base*10}px ${props.theme.space.base*8}px`,props=>retrieveComponentStyles(COMPONENT_ID$g,props));StyledFooter.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$f='modals.footer_item';const StyledFooterItem=styled__default["default"].span.attrs({'data-garden-id':COMPONENT_ID$f,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledFooterItem",componentId:"sc-1mb76hl-0"})(["display:flex;margin-",":","px;min-width:0;&:first-child{margin-",":0;}",";"],props=>props.theme.rtl?'right':'left',props=>props.theme.space.base*5,props=>props.theme.rtl?'right':'left',props=>retrieveComponentStyles(COMPONENT_ID$f,props));StyledFooterItem.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$e='modals.header';const StyledHeader=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$e,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledHeader",componentId:"sc-1787r9v-0"})(["display:block;position:",";margin:0;border-bottom:"," ",";padding:",";","  line-height:",";color:",";font-size:",";font-weight:",";",";"],props=>props.isDanger&&'relative',props=>props.theme.borders.sm,getColor('neutralHue',200),props=>`${props.theme.space.base*5}px ${props.theme.space.base*10}px`,props=>props.isCloseButtonPresent&&`padding-${props.theme.rtl?'left':'right'}: ${props.theme.space.base*(BASE_MULTIPLIERS$1.size+BASE_MULTIPLIERS$1.side+2)}px;`,props=>getLineHeight(props.theme.lineHeights.md,props.theme.fontSizes.md),props=>props.isDanger?getColor('dangerHue',600,props.theme):props.theme.colors.foreground,props=>props.theme.fontSizes.md,props=>props.theme.fontWeights.semibold,props=>retrieveComponentStyles(COMPONENT_ID$e,props));StyledHeader.defaultProps={theme:DEFAULT_THEME};var _g,_circle;function _extends$1$1(){_extends$1$1=Object.assign?Object.assign.bind():function(target){for(var i=1;i<arguments.length;i++){var source=arguments[i];for(var key in source){if(Object.prototype.hasOwnProperty.call(source,key)){target[key]=source[key];}}}return target;};return _extends$1$1.apply(this,arguments);}var SvgAlertErrorStroke=function SvgAlertErrorStroke(props){return/*#__PURE__*/React__namespace.createElement("svg",_extends$1$1({xmlns:"http://www.w3.org/2000/svg",width:16,height:16,focusable:"false",viewBox:"0 0 16 16","aria-hidden":"true"},props),_g||(_g=/*#__PURE__*/React__namespace.createElement("g",{fill:"none",stroke:"currentColor"},/*#__PURE__*/React__namespace.createElement("circle",{cx:7.5,cy:8.5,r:7}),/*#__PURE__*/React__namespace.createElement("path",{strokeLinecap:"round",d:"M7.5 4.5V9"}))),_circle||(_circle=/*#__PURE__*/React__namespace.createElement("circle",{cx:7.5,cy:12,r:1,fill:"currentColor"})));};const StyledDangerIcon=styled__default["default"](SvgAlertErrorStroke).withConfig({displayName:"StyledDangerIcon",componentId:"sc-1kwbb39-0"})(["position:absolute;top:","px;",":",";"],props=>props.theme.space.base*5.5,props=>props.theme.rtl?'right':'left',props=>`${props.theme.space.base*4}px`);StyledDangerIcon.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$d='modals.modal';const animationName=styled.keyframes(["0%{transform:scale(0);opacity:0;}50%{transform:scale(1.05);}100%{opacity:1;}"]);const animationStyles$2=props=>{if(props.isAnimated){return styled.css(["animation:"," 0.3s ease-in;"],animationName);}return'';};const boxShadow$1=props=>{const{theme}=props;const{space,shadows}=theme;const offsetY=`${space.base*5}px`;const blurRadius=`${space.base*7}px`;const color=getColor('neutralHue',800,theme,0.35);return shadows.lg(offsetY,blurRadius,color);};const sizeStyles$1$1=props=>{return styled.css(["","{width:",";}"],mediaQuery('up',props.isLarge?'md':'sm',props.theme),props.isLarge?props.theme.breakpoints.md:props.theme.breakpoints.sm);};const StyledModal=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$d,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledModal",componentId:"sc-1pe1axu-0"})(["display:flex;position:fixed;flex-direction:column;animation-delay:0.01s;margin:",";border-radius:",";box-shadow:",";background-color:",";min-height:60px;max-height:calc(100vh - ","px);overflow:auto;direction:",";",";",";&:focus{outline:none;}@media (max-height:399px){top:","px;bottom:auto;margin-bottom:","px;max-height:none;}@media screen and (-ms-high-contrast:active),screen and (-ms-high-contrast:none){right:",";bottom:",";transform:",";}",";"],props=>props.isCentered?'0':`${props.theme.space.base*12}px`,props=>props.theme.borderRadii.md,boxShadow$1,props=>props.theme.colors.background,props=>props.theme.space.base*24,props=>props.theme.rtl&&'rtl',animationStyles$2,sizeStyles$1$1,props=>props.theme.space.base*6,props=>props.theme.space.base*6,props=>props.isCentered&&'50%',props=>props.isCentered&&'50%',props=>props.isCentered&&'translate(50%, 50%)',props=>retrieveComponentStyles(COMPONENT_ID$d,props));StyledModal.propTypes={isLarge:propTypes.exports.bool,isAnimated:propTypes.exports.bool};StyledModal.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$c='modals.tooltip_modal.backdrop';const StyledTooltipModalBackdrop=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$c,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledTooltipModalBackdrop",componentId:"sc-1yaomgq-0"})(["position:fixed;inset:0;z-index:400;overflow:hidden;-webkit-overflow-scrolling:touch;font-family:",";direction:",";&.garden-tooltip-modal-transition-exit-active{pointer-events:none;}&.garden-tooltip-modal-transition-exit-active div{transition:opacity 200ms;opacity:0;}",";"],props=>props.theme.fonts.system,props=>props.theme.rtl&&'rtl',props=>retrieveComponentStyles(COMPONENT_ID$c,props));StyledTooltipModalBackdrop.defaultProps={theme:DEFAULT_THEME};function getPopperPlacement(gardenPlacement){const gardenToPopperMapping={auto:'auto',top:'top','top-start':'top-start','top-end':'top-end',bottom:'bottom','bottom-start':'bottom-start','bottom-end':'bottom-end',end:'right','end-top':'right-start','end-bottom':'right-end',start:'left','start-top':'left-start','start-bottom':'left-end'};return gardenToPopperMapping[gardenPlacement];}function getRtlPopperPlacement(gardenPlacement){const rtlPlacementMappings={left:'right','left-start':'right-start','left-end':'right-end','top-start':'top-end','top-end':'top-start',right:'left','right-start':'left-start','right-end':'left-end','bottom-start':'bottom-end','bottom-end':'bottom-start'};const popperPlacement=getPopperPlacement(gardenPlacement);return rtlPlacementMappings[popperPlacement]||popperPlacement;}function getArrowPosition(popperPlacement){const arrowPositionMappings={top:'bottom','top-start':'bottom-left','top-end':'bottom-right',right:'left','right-start':'left-top','right-end':'left-bottom',bottom:'top','bottom-start':'top-left','bottom-end':'top-right',left:'right','left-start':'right-top','left-end':'right-bottom'};return arrowPositionMappings[popperPlacement]||'top';}function getMenuPosition(popperPlacement){return popperPlacement?popperPlacement.split('-')[0]:'bottom';}const StyledTooltipWrapper=styled__default["default"].div.attrs(props=>({className:props.isAnimated&&'is-animated'})).withConfig({displayName:"StyledTooltipWrapper",componentId:"sc-1xk05kf-0"})(["",";"],props=>menuStyles(getMenuPosition(props.placement),{theme:props.theme,hidden:false,margin:'0',zIndex:props.zIndex,animationModifier:'.is-animated'}));StyledTooltipWrapper.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$b='modals.tooltip_modal';const StyledTooltipModal=styled__default["default"].div.attrs(props=>({'data-garden-id':COMPONENT_ID$b,'data-garden-version':'8.70.0',className:props.isAnimated&&'is-animated'})).withConfig({displayName:"StyledTooltipModal",componentId:"sc-42ubfr-0"})(["padding:","px;width:400px;",";",";"],props=>props.theme.space.base*5,props=>{const computedArrowStyles=arrowStyles(getArrowPosition(props.placement),{size:`${props.theme.space.base*2}px`,inset:'1px',animationModifier:'.is-animated'});if(props.isAnimated){return props.hasArrow&&props.transitionState==='entered'&&computedArrowStyles;}return props.hasArrow&&computedArrowStyles;},props=>retrieveComponentStyles(COMPONENT_ID$b,props));StyledTooltipModal.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$a='modals.tooltip_modal.title';const sizeStyles$2=props=>`
...
`;const StyledTooltipModalTitle=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$a,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledTooltipModalTitle",componentId:"sc-11xjgjs-0"})(["margin:0;color:",";font-weight:",";",";",";"],props=>props.theme.colors.foreground,props=>props.theme.fontWeights.semibold,props=>sizeStyles$2(props),props=>retrieveComponentStyles(COMPONENT_ID$a,props));StyledTooltipModalTitle.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$9='modals.tooltip_modal.body';const StyledTooltipModalBody=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$9,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledTooltipModalBody",componentId:"sc-195dkzj-0"})(["display:block;margin:0;padding-top:","px;line-height:",";color:",";font-size:",";",";"],props=>props.theme.space.base*1.5,props=>getLineHeight(props.theme.lineHeights.md,props.theme.fontSizes.md),props=>props.theme.colors.foreground,props=>props.theme.fontSizes.md,props=>retrieveComponentStyles(COMPONENT_ID$9,props));StyledTooltipModalBody.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$8='modals.tooltip_modal.footer';const StyledTooltipModalFooter=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$8,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledTooltipModalFooter",componentId:"sc-fm36a9-0"})(["display:flex;flex-shrink:0;align-items:center;justify-content:flex-end;padding-top:","px;",";"],p=>p.theme.space.base*5,props=>retrieveComponentStyles(COMPONENT_ID$8,props));StyledTooltipModalFooter.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$7='modals.tooltip_modal.footer_item';const StyledTooltipModalFooterItem=styled__default["default"](StyledFooterItem).attrs({'data-garden-id':COMPONENT_ID$7,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledTooltipModalFooterItem",componentId:"sc-1nahj6p-0"})(["",";"],props=>retrieveComponentStyles(COMPONENT_ID$7,props));StyledTooltipModalFooterItem.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$6$1='modals.tooltip_modal.close';const StyledTooltipModalClose=styled__default["default"](StyledClose).attrs({'data-garden-id':COMPONENT_ID$6$1,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledTooltipModalClose",componentId:"sc-1h2ke3q-0"})(["top:","px;",":",";width:","px;height:","px;",";"],props=>props.theme.space.base*3.5,props=>props.theme.rtl?'left':'right',props=>`${props.theme.space.base*3}px`,props=>props.theme.space.base*8,props=>props.theme.space.base*8,props=>retrieveComponentStyles(COMPONENT_ID$6$1,props));StyledTooltipModalClose.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$5$2='modals.drawer_modal';const DRAWER_WIDTH=380;const boxShadow=props=>{const{theme}=props;const{space,shadows}=theme;const offsetY=`${space.base*5}px`;const blurRadius=`${space.base*7}px`;const color=getColor('neutralHue',800,theme,0.35);return shadows.lg(offsetY,blurRadius,color);};const StyledDrawerModal=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$5$2,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledDrawerModal",componentId:"sc-i1sake-0"})(["display:flex;position:fixed;top:0;",":0;flex-direction:column;z-index:500;box-shadow:",";background:",";width:","px;height:100%;overflow:auto;-webkit-overflow-scrolling:touch;font-family:",";direction:",";&.garden-drawer-transition-enter{transform:translateX(","px);}&.garden-drawer-transition-enter-active{transform:translateX(0);transition:transform 0.25s ease-in-out;}&.garden-drawer-transition-exit-active{transform:translateX(","px);transition:transform 0.25s ease-in-out;}&:focus{outline:none;}",";"],props=>props.theme.rtl?'left':'right',boxShadow,props=>props.theme.colors.background,DRAWER_WIDTH,props=>props.theme.fonts.system,props=>props.theme.rtl&&'rtl',props=>props.theme.rtl?-DRAWER_WIDTH:DRAWER_WIDTH,props=>props.theme.rtl?-DRAWER_WIDTH:DRAWER_WIDTH,props=>retrieveComponentStyles(COMPONENT_ID$5$2,props));StyledDrawerModal.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$4$2='modals.drawer_modal.close';const BASE_MULTIPLIERS={top:BASE_MULTIPLIERS$1.top,side:2,size:BASE_MULTIPLIERS$1.size};const StyledDrawerModalClose=styled__default["default"](StyledClose).attrs({'data-garden-id':COMPONENT_ID$4$2,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledDrawerModalClose",componentId:"sc-hrnaom-0"})(["",":",";",";"],props=>props.theme.rtl?'left':'right',props=>`${props.theme.space.base*BASE_MULTIPLIERS.side}px`,props=>retrieveComponentStyles(COMPONENT_ID$4$2,props));StyledDrawerModalClose.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$3$2='modals.drawer_modal.header';const StyledDrawerModalHeader=styled__default["default"](StyledHeader).attrs({'data-garden-id':COMPONENT_ID$3$2,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledDrawerModalHeader",componentId:"sc-1u04rqw-0"})(["padding:","px;","  ",";"],props=>props.theme.space.base*5,props=>props.isCloseButtonPresent&&`padding-${props.theme.rtl?'left':'right'}: ${props.theme.space.base*(BASE_MULTIPLIERS.size+BASE_MULTIPLIERS.side+2)}px;`,props=>retrieveComponentStyles(COMPONENT_ID$3$2,props));StyledDrawerModalHeader.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$2$2='modals.drawer_modal.body';const StyledDrawerModalBody=styled__default["default"](StyledBody).attrs({'data-garden-id':COMPONENT_ID$2$2,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledDrawerModalBody",componentId:"sc-yafe2y-0"})(["padding:","px;",";"],props=>props.theme.space.base*5,props=>retrieveComponentStyles(COMPONENT_ID$2$2,props));StyledDrawerModalBody.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$1$2='modals.drawer_modal.footer';const StyledDrawerModalFooter=styled__default["default"].div.attrs({'data-garden-id':COMPONENT_ID$1$2,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledDrawerModalFooter",componentId:"sc-17if4ka-0"})(["display:flex;flex-shrink:0;justify-content:flex-end;border-top:",";padding:","px;",";"],props=>`${props.theme.borders.sm} ${getColor('neutralHue',200,props.theme)}`,props=>props.theme.space.base*5,props=>retrieveComponentStyles(COMPONENT_ID$1$2,props));StyledDrawerModalFooter.defaultProps={theme:DEFAULT_THEME};const COMPONENT_ID$k='modals.drawer_modal.footer_item';const StyledDrawerModalFooterItem=styled__default["default"](StyledFooterItem).attrs({'data-garden-id':COMPONENT_ID$k,'data-garden-version':'8.70.0'}).withConfig({displayName:"StyledDrawerModalFooterItem",componentId:"sc-1vbl885-0"})(["",";"],props=>retrieveComponentStyles(COMPONENT_ID$k,props));StyledDrawerModalFooterItem.defaultProps={theme:DEFAULT_THEME};const ModalsContext=React.createContext(undefined);const useModalContext=()=>{const context=React.useContext(ModalsContext);if(context===undefined){throw new Error('useModalContext must be used within a ModalsContext.Provider');}return context;};const isOverflowing=element=>{const doc=ownerDocument(element);const win=ownerWindow(doc);const isBody=element&&element.tagName.toLowerCase()==='body';if(!isWindow(doc)&&!isBody){return element.scrollHeight>element.clientHeight;}const style=win.getComputedStyle(doc.body);const marginLeft=parseInt(style.getPropertyValue('margin-left'),10);const marginRight=parseInt(style.getPropertyValue('margin-right'),10);return marginLeft+doc.body.clientWidth+marginRight<win.innerWidth;};const Modal=React.forwardRef((_ref,ref)=>{let{backdropProps,children,onClose,isLarge,isCentered,isAnimated,id,appendToNode,focusOnMount,restoreFocus,...modalProps}=_ref;const theme=React.useContext(styled.ThemeContext);const modalRef=React.useRef(null);const environment=useDocument(theme);const[isCloseButtonPresent,setIsCloseButtonPresent]=React.useState(false);const[hasHeader,setHasHeader]=React.useState(false);const{getBackdropProps,getModalProps,getTitleProps,getContentProps,getCloseProps}=useModal({idPrefix:id,onClose,modalRef,focusOnMount,restoreFocus});useFocusVisible({scope:modalRef,relativeDocument:environment});React.useEffect(()=>{if(!environment){return undefined;}const htmlElement=environment.querySelector('html');const bodyElement=environment.querySelector('body');let previousHtmlOverflow;let previousBodyPaddingRight;if(bodyElement){if(isOverflowing(bodyElement)){const scrollbarSize$1=scrollbarSize();const bodyPaddingRight=parseInt(style(bodyElement,'paddingRight')||'0',10);previousBodyPaddingRight=bodyElement.style.paddingRight;bodyElement.style.paddingRight=`${bodyPaddingRight+scrollbarSize$1}px`;}if(htmlElement){previousHtmlOverflow=htmlElement.style.overflow;htmlElement.style.overflow='hidden';}return()=>{if(htmlElement){htmlElement.style.overflow=previousHtmlOverflow;}bodyElement.style.paddingRight=previousBodyPaddingRight;};}return undefined;},[environment]);const rootNode=React.useMemo(()=>{if(appendToNode){return appendToNode;}if(environment){return environment.body;}return undefined;},[appendToNode,environment]);const value=React.useMemo(()=>({isLarge,isCloseButtonPresent,hasHeader,setHasHeader,getTitleProps,getContentProps,getCloseProps,setIsCloseButtonPresent}),[isLarge,hasHeader,isCloseButtonPresent,getTitleProps,getContentProps,getCloseProps]);const modalContainerProps=getModalProps({'aria-describedby':undefined,...(hasHeader?{}:{'aria-labelledby':undefined})});const attribute=hasHeader?'aria-labelledby':'aria-label';const defaultValue=hasHeader?modalContainerProps['aria-labelledby']:'Modal dialog';const labelValue=hasHeader?modalContainerProps['aria-labelledby']:modalProps['aria-label'];const ariaProps={[attribute]:useText(Modal,{[attribute]:labelValue},attribute,defaultValue)};if(!rootNode){return null;}return ReactDOM.createPortal(React__default["default"].createElement(ModalsContext.Provider,{value:value},React__default["default"].createElement(StyledBackdrop,_extends$2$1({isCentered:isCentered,isAnimated:isAnimated},getBackdropProps(backdropProps)),React__default["default"].createElement(StyledModal,_extends$2$1({isCentered:isCentered,isAnimated:isAnimated,isLarge:isLarge},modalContainerProps,ariaProps,modalProps,{ref:mergeRefs([ref,modalRef])}),children))),rootNode);});Modal.displayName='Modal';Modal.propTypes={backdropProps:propTypes.exports.object,isLarge:propTypes.exports.bool,isAnimated:propTypes.exports.bool,isCentered:propTypes.exports.bool,focusOnMount:propTypes.exports.bool,restoreFocus:propTypes.exports.bool,onClose:propTypes.exports.func,appendToNode:propTypes.exports.any};Modal.defaultProps={isAnimated:true,isCentered:true};const Body$2=React.forwardRef((props,ref)=>{const{getContentProps}=useModalContext();return React__default["default"].createElement(StyledBody,_extends$2$1({},getContentProps(props),{ref:ref}));});Body$2.displayName='Body';var _path$2;function _extends$4(){_extends$4=Object.assign?Object.assign.bind():function(target){for(var i=1;i<arguments.length;i++){var source=arguments[i];for(var key in source){if(Object.prototype.hasOwnProperty.call(source,key)){target[key]=source[key];}}}return target;};return _extends$4.apply(this,arguments);}var SvgXStroke=function SvgXStroke(props){return/*#__PURE__*/React__namespace.createElement("svg",_extends$4({xmlns:"http://www.w3.org/2000/svg",width:16,height:16,focusable:"false",viewBox:"0 0 16 16","aria-hidden":"true"},props),_path$2||(_path$2=/*#__PURE__*/React__namespace.createElement("path",{stroke:"currentColor",strokeLinecap:"round",d:"M3 13L13 3m0 10L3 3"})));};const Close$2=React.forwardRef((props,ref)=>{const{getCloseProps,setIsCloseButtonPresent}=useModalContext();React.useEffect(()=>{setIsCloseButtonPresent(true);return()=>setIsCloseButtonPresent(false);});const ariaLabel=useText(Close$2,props,'aria-label','Close modal');return React__default["default"].createElement(StyledClose,_extends$2$1({},getCloseProps({...props,'aria-label':ariaLabel}),{ref:ref}),React__default["default"].createElement(SvgXStroke,null));});Close$2.displayName='Close';const Footer$2=React__default["default"].forwardRef((props,ref)=>{const{isLarge}=useModalContext();return React__default["default"].createElement(StyledFooter,_extends$2$1({ref:ref,isLarge:isLarge},props));});Footer$2.displayName='Footer';const FooterItem$2=React__default["default"].forwardRef((props,ref)=>React__default["default"].createElement(StyledFooterItem,_extends$2$1({ref:ref},props)));FooterItem$2.displayName='FooterItem';const Header$1=React.forwardRef((_ref,ref)=>{let{children,tag,...other}=_ref;const{isCloseButtonPresent,hasHeader,setHasHeader,getTitleProps}=useModalContext();React.useEffect(()=>{if(!hasHeader&&setHasHeader){setHasHeader(true);}return()=>{if(hasHeader&&setHasHeader){setHasHeader(false);}};},[hasHeader,setHasHeader]);return React__default["default"].createElement(StyledHeader,_extends$2$1({},getTitleProps(other),{as:tag,isCloseButtonPresent:isCloseButtonPresent,ref:ref}),other.isDanger&&React__default["default"].createElement(StyledDangerIcon,null),children);});Header$1.displayName='Header';Header$1.propTypes={isDanger:propTypes.exports.bool,tag:propTypes.exports.any};Header$1.defaultProps={tag:'div'};const TooltipModalContext=React.createContext(undefined);const useTooltipModalContext=()=>{const context=React.useContext(TooltipModalContext);if(context===undefined){throw new Error('Element must be used within a TooltipModal component.');}return context;};const TitleComponent=React.forwardRef((_ref,ref)=>{let{children,tag,...other}=_ref;const{getTitleProps,hasTitle,setHasTitle}=useTooltipModalContext();React.useEffect(()=>{if(!hasTitle&&setHasTitle){setHasTitle(true);}return()=>{if(hasTitle&&setHasTitle){setHasTitle(false);}};},[hasTitle,setHasTitle]);return React__default["default"].createElement(StyledTooltipModalTitle,_extends$2$1({},getTitleProps(other),{as:tag,ref:ref}),children);});TitleComponent.displayName='TooltipModal.Title';TitleComponent.propTypes={tag:propTypes.exports.any};TitleComponent.defaultProps={tag:'div'};const Title=TitleComponent;const BodyComponent$1=React.forwardRef((props,ref)=>{const{getContentProps}=useTooltipModalContext();return React__default["default"].createElement(StyledTooltipModalBody,_extends$2$1({},getContentProps(props),{ref:ref}));});BodyComponent$1.displayName='TooltipModal.Body';const Body$1=BodyComponent$1;const CloseComponent$1=React.forwardRef((props,ref)=>{const{getCloseProps}=useTooltipModalContext();const ariaLabel=useText(CloseComponent$1,props,'aria-label','Close tooltip');return React__default["default"].createElement(StyledTooltipModalClose,_extends$2$1({},getCloseProps({...props,'aria-label':ariaLabel}),{ref:ref}),React__default["default"].createElement(SvgXStroke,null));});CloseComponent$1.displayName='TooltipModal.Close';const Close$1=CloseComponent$1;const FooterComponent$1=React.forwardRef((props,ref)=>React__default["default"].createElement(StyledTooltipModalFooter,_extends$2$1({ref:ref},props)));FooterComponent$1.displayName='TooltipModal.Footer';const Footer$1=FooterComponent$1;const FooterItemComponent$1=React.forwardRef((props,ref)=>React__default["default"].createElement(StyledTooltipModalFooterItem,_extends$2$1({ref:ref},props)));FooterItemComponent$1.displayName='TooltipModal.FooterItem';const FooterItem$1=FooterItemComponent$1;const TooltipModalComponent=React__default["default"].forwardRef((_ref,ref)=>{let{appendToNode,referenceElement,popperModifiers,placement,onClose,hasArrow,isAnimated,zIndex,backdropProps,focusOnMount,restoreFocus,id,...props}=_ref;const theme=React.useContext(styled.ThemeContext);const previousReferenceElementRef=React.useRef();const modalRef=React.useRef(null);const transitionRef=React.useRef(null);const[popperElement,setPopperElement]=React.useState();const[hasTitle,setHasTitle]=React.useState(false);const{getTitleProps,getCloseProps,getContentProps,getBackdropProps,getModalProps}=useModal({idPrefix:id,onClose,modalRef,focusOnMount,restoreFocus:false});React.useEffect(()=>{if(!referenceElement&&previousReferenceElementRef.current&&restoreFocus){previousReferenceElementRef.current.focus();}previousReferenceElementRef.current=referenceElement;},[referenceElement,restoreFocus]);const popperPlacement=React.useMemo(()=>theme.rtl?getRtlPopperPlacement(placement):getPopperPlacement(placement),[placement,theme.rtl]);const{styles,attributes,state}=usePopper(referenceElement,popperElement,{placement:popperPlacement,modifiers:[{name:'offset',options:{offset:[0,theme.space.base*3]}},...(popperModifiers||[])]});const modalProps=getModalProps({'aria-describedby':undefined,...(hasTitle?{}:{'aria-labelledby':undefined})});const attribute=hasTitle?'aria-labelledby':'aria-label';const defaultValue=hasTitle?modalProps['aria-labelledby']:'Modal dialog';const labelValue=hasTitle?modalProps['aria-labelledby']:props['aria-label'];const ariaProps={[attribute]:useText(TooltipModalComponent,{[attribute]:labelValue},attribute,defaultValue)};const value={hasTitle,setHasTitle,getTitleProps,getContentProps,getCloseProps};const Node=React__default["default"].createElement(CSSTransition,{unmountOnExit:true,timeout:isAnimated?200:0,in:Boolean(referenceElement),classNames:isAnimated?'garden-tooltip-modal-transition':'',nodeRef:transitionRef},transitionState=>{return React__default["default"].createElement(TooltipModalContext.Provider,{value:value},React__default["default"].createElement(StyledTooltipModalBackdrop,_extends$2$1({},getBackdropProps(),backdropProps,{ref:transitionRef}),React__default["default"].createElement(StyledTooltipWrapper,_extends$2$1({ref:setPopperElement,style:styles.popper,placement:state?state.placement:undefined,zIndex:zIndex,isAnimated:isAnimated},attributes.popper),React__default["default"].createElement(StyledTooltipModal,_extends$2$1({transitionState:transitionState,placement:state?state.placement:'top',hasArrow:hasArrow,isAnimated:isAnimated},modalProps,ariaProps,props,{ref:mergeRefs([modalRef,ref])})))));});return appendToNode?ReactDOM.createPortal(Node,appendToNode):Node;});TooltipModalComponent.displayName='TooltipModal';TooltipModalComponent.defaultProps={placement:'auto',hasArrow:true,focusOnMount:true,restoreFocus:true};TooltipModalComponent.propTypes={appendToNode:propTypes.exports.any,referenceElement:propTypes.exports.any,popperModifiers:propTypes.exports.any,placement:propTypes.exports.any,isAnimated:propTypes.exports.bool,hasArrow:propTypes.exports.bool,zIndex:propTypes.exports.number,onClose:propTypes.exports.func,backdropProps:propTypes.exports.any,focusOnMount:propTypes.exports.bool,restoreFocus:propTypes.exports.bool};const TooltipModal=TooltipModalComponent;TooltipModal.Body=Body$1;TooltipModal.Close=Close$1;TooltipModal.Footer=Footer$1;TooltipModal.FooterItem=FooterItem$1;TooltipModal.Title=Title;const HeaderComponent=React.forwardRef((_ref,ref)=>{let{tag,...other}=_ref;const{isCloseButtonPresent,hasHeader,setHasHeader,getTitleProps}=useModalContext();React.useEffect(()=>{if(!hasHeader&&setHasHeader){setHasHeader(true);}return()=>{if(hasHeader&&setHasHeader){setHasHeader(false);}};},[hasHeader,setHasHeader]);return React__default["default"].createElement(StyledDrawerModalHeader,_extends$2$1({},getTitleProps(other),{as:tag,isCloseButtonPresent:isCloseButtonPresent,ref:ref}));});HeaderComponent.displayName='DrawerModal.Header';HeaderComponent.propTypes={tag:propTypes.exports.any};HeaderComponent.defaultProps={tag:'div'};const Header$2=HeaderComponent;const BodyComponent=React.forwardRef((props,ref)=>{const{getContentProps}=useModalContext();return React__default["default"].createElement(StyledDrawerModalBody,_extends$2$1({},getContentProps(props),{ref:ref}),props.children);});BodyComponent.displayName='DrawerModal.Body';const Body$3=BodyComponent;const CloseComponent=React.forwardRef((props,ref)=>{const{getCloseProps,setIsCloseButtonPresent}=useModalContext();React.useEffect(()=>{setIsCloseButtonPresent(true);return()=>setIsCloseButtonPresent(false);});const ariaLabel=useText(CloseComponent,props,'aria-label','Close drawer');return React__default["default"].createElement(StyledDrawerModalClose,_extends$2$1({},getCloseProps({...props,'aria-label':ariaLabel}),{ref:ref}),React__default["default"].createElement(SvgXStroke,null));});CloseComponent.displayName='DrawerModal.Close';const Close=CloseComponent;const FooterComponent=React.forwardRef((props,ref)=>React__default["default"].createElement(StyledDrawerModalFooter,_extends$2$1({ref:ref},props)));FooterComponent.displayName='DrawerModal.Footer';const Footer=FooterComponent;const FooterItemComponent=React.forwardRef((props,ref)=>React__default["default"].createElement(StyledDrawerModalFooterItem,_extends$2$1({ref:ref},props)));FooterItemComponent.displayName='DrawerModal.FooterItem';const FooterItem=FooterItemComponent;const DrawerModalComponent=React.forwardRef((_ref,ref)=>{let{id,isOpen,onClose,backdropProps,appendToNode,focusOnMount,restoreFocus,...props}=_ref;const modalRef=React.useRef(null);const transitionRef=React.useRef(null);const triggerRef=React.useRef(null);const theme=React.useContext(styled.ThemeContext);const environment=useDocument(theme);const[isCloseButtonPresent,setIsCloseButtonPresent]=React.useState(false);const[hasHeader,setHasHeader]=React.useState(false);useFocusVisible({scope:modalRef,relativeDocument:modalRef.current});const{getTitleProps,getCloseProps,getContentProps,getBackdropProps,getModalProps}=useModal({idPrefix:id,modalRef,focusOnMount:false,restoreFocus:false,environment,onClose});React.useEffect(()=>{if(environment){if(isOpen&&modalRef.current){if(restoreFocus){triggerRef.current=activeElement(environment);}if(focusOnMount){modalRef.current.focus();}}if(!isOpen&&triggerRef.current){triggerRef.current.focus();}}return()=>{if(!(restoreFocus&&isOpen)){triggerRef.current=null;}};},[environment,restoreFocus,focusOnMount,isOpen]);React.useEffect(()=>{if(!environment){return undefined;}const htmlElement=environment.querySelector('html');let previousHtmlOverflow;if(htmlElement&&isOpen){previousHtmlOverflow=htmlElement.style.overflow;htmlElement.style.overflow='hidden';}return()=>{if(htmlElement&&isOpen){htmlElement.style.overflow=previousHtmlOverflow;}};},[environment,isOpen]);const rootNode=React.useMemo(()=>{if(appendToNode){return appendToNode;}if(environment){return environment.body;}return undefined;},[appendToNode,environment]);const value=React.useMemo(()=>({isCloseButtonPresent,hasHeader,setHasHeader,getTitleProps,getContentProps,getCloseProps,setIsCloseButtonPresent}),[isCloseButtonPresent,hasHeader,getTitleProps,getContentProps,getCloseProps]);const modalProps=getModalProps({'aria-describedby':undefined,...(hasHeader?{}:{'aria-labelledby':undefined})});const attribute=hasHeader?'aria-labelledby':'aria-label';const defaultValue=hasHeader?modalProps['aria-labelledby']:'Modal dialog';const labelValue=hasHeader?modalProps['aria-labelledby']:props['aria-label'];const ariaProps={[attribute]:useText(DrawerModalComponent,{[attribute]:labelValue},attribute,defaultValue)};if(!rootNode){return null;}return ReactDOM__default["default"].createPortal(React__default["default"].createElement(ModalsContext.Provider,{value:value},React__default["default"].createElement(CSSTransition,{in:isOpen,timeout:250,unmountOnExit:true,classNames:"garden-drawer-transition",nodeRef:transitionRef},React__default["default"].createElement(StyledBackdrop,_extends$2$1({isAnimated:true},getBackdropProps(backdropProps)),React__default["default"].createElement(StyledDrawerModal,_extends$2$1({},modalProps,ariaProps,props,{ref:mergeRefs([ref,modalRef,transitionRef])}))))),rootNode);});DrawerModalComponent.displayName='DrawerModal';DrawerModalComponent.propTypes={backdropProps:propTypes.exports.object,focusOnMount:propTypes.exports.bool,restoreFocus:propTypes.exports.bool,onClose:propTypes.exports.func,appendToNode:propTypes.exports.any,isOpen:propTypes.exports.bool};DrawerModalComponent.defaultProps={focusOnMount:true,restoreFocus:true};const DrawerModal$1=DrawerModalComponent;DrawerModal$1.Body=Body$3;DrawerModal$1.Close=Close;DrawerModal$1.Footer=Footer;DrawerModal$1.FooterItem=FooterItem;DrawerModal$1.Header=Header$2;/**
 * Copyright Zendesk, Inc.
 *
 * Use of this source code is governed under the Apache License, Version 2.0
 * found at http://www.apache.org/licenses/LICENSE-2.0.
...
onClick:autoLoadable?processClick:onClick,primary:primary}));}))(templateObject_9||(templateObject_9=__makeTemplateObject([""],[""])));var LoaderContainer=styled__default["default"](FlexBox)(templateObject_10||(templateObject_10=__makeTemplateObject(["\n  min-height: 30px;\n"],["\n  min-height: 30px;\n"])));var IconButtonTextContainer=styled__default["default"].span(templateObject_11||(templateObject_11=__makeTemplateObject(["\n  flex: 1;\n"],["\n  flex: 1;\n"])));var templateObject_1$4,templateObject_2$2,templateObject_3$2,templateObject_4$2,templateObject_5$2,templateObject_6$2,templateObject_7,templateObject_8,templateObject_9,templateObject_10,templateObject_11;/* Size key name represents all devices with a pixel width up to the number
 specified */var REGULAR_BREAKPOINTS_PX={phoneSize:575,tabletSize:767,largeTabletSize:991,smallComputerSize:1199};var WIDE_BREAKPOINTS_PX={phoneSize:560,tabletSize:840,largeTabletSize:991,smallComputerSize:1350};var breakpoints=function(_a){var _b=_a.wideLayout,wideLayout=_b===void 0?false:_b;return wideLayout?WIDE_BREAKPOINTS_PX:REGULAR_BREAKPOINTS_PX;};var mediaQueries=function(wideLayout){var _a=breakpoints({wideLayout:wideLayout}),phoneSize=_a.phoneSize,tabletSize=_a.tabletSize,largeTabletSize=_a.largeTabletSize,smallComputerSize=_a.smallComputerSize;return{forPhones:function(styles){return styled.css(templateObject_1$3||(templateObject_1$3=__makeTemplateObject(["\n      @media screen and (max-width: ",") {\n        ","\n      }\n    "],["\n      @media screen and (max-width: ",") {\n        ","\n      }\n    "])),unit(phoneSize),styles);},forTablets:function(styles){return styled.css(templateObject_2$1||(templateObject_2$1=__makeTemplateObject(["\n      @media screen and (max-width: ",") {\n        ","\n      }\n    "],["\n      @media screen and (max-width: ",") {\n        ","\n      }\n    "])),unit(tabletSize),styles);},forTabletsAndUp:function(styles){return styled.css(templateObject_3$1||(templateObject_3$1=__makeTemplateObject(["\n      @media screen and (min-width: ",") {\n        ","\n      }\n    "],["\n      @media screen and (min-width: ",") {\n        ","\n      }\n    "])),unit(phoneSize+1),styles);},forLargeTabletsAndUp:function(styles){return styled.css(templateObject_4$1||(templateObject_4$1=__makeTemplateObject(["\n      @media (min-width: ",") {\n        ","\n      }\n    "],["\n      @media (min-width: ",") {\n        ","\n      }\n    "])),unit(tabletSize+1),styles);},forSmallComputersAndUp:function(styles){return styled.css(templateObject_5$1||(templateObject_5$1=__makeTemplateObject(["\n      @media screen and (min-width: ",") {\n        ","\n      }\n    "],["\n      @media screen and (min-width: ",") {\n        ","\n      }\n    "])),unit(largeTabletSize+1),styles);},forLargeComputers:function(styles){return styled.css(templateObject_6$1||(templateObject_6$1=__makeTemplateObject(["\n      @media screen and (min-width: ",") {\n        ","\n      }\n    "],["\n      @media screen and (min-width: ",") {\n        ","\n      }\n    "])),unit(smallComputerSize+1),styles);}};};var unit=function(px){return"".concat(px,"px");};var templateObject_1$3,templateObject_2$1,templateObject_3$1,templateObject_4$1,templateObject_5$1,templateObject_6$1;var useTheme=function(){return React.useContext(styled.ThemeContext);};// export const setThemeContext = (ctx :)
var MODAL_SIZES={FULL_SCREEN:"FULL_SCREEN",LG:"LG",SM:"SM",XL:"XL"};var Header=styled__default["default"](Header$1).attrs(function(_a){var danger=_a.danger,success=_a.success,warning=_a.warning,theme=_a.theme;var color=theme.styles.textColorPrimary;if(danger)color=theme.styles.colorDanger;if(success)color=theme.styles.colorSuccess;if(warning)color=theme.styles.colorWarning;return{color:color};})(templateObject_1$2||(templateObject_1$2=__makeTemplateObject(["\n  ","\n"],["\n  ","\n"])),textWithColor);var createButtons=function(buttons,closeModal,shouldDisable,autoClose,adjustModalState){return buttons.map(function(b){var _a=b.props,onClick=_a.onClick,disabled=_a.disabled,disableable=_a.disableable;return jsxRuntime.jsx(FooterItem$2,{children:React__default["default"].cloneElement(b,{onClick:function(){return __awaiter(void 0,void 0,void 0,function(){return __generator(this,function(_a){switch(_a.label){case 0:adjustModalState();return[4/*yield*/,onClick()];case 1:_a.sent();if(autoClose)closeModal();return[2/*return*/];}});});},disabled:disableable?shouldDisable:disabled})},b.key);});};exports.Modal=function(_a){var _b,_c;var blurBackdrop=_a.blurBackdrop,isVisible=_a.isVisible,closeModal=_a.closeModal,disableActions=_a.disableActions,modalContent=_a.modalContent,className=_a.className;var isProcessing=(_b=__read(React.useState(false),2),_b[0]),setIsProcessingTo=_b[1];var theme=useTheme();if(!isVisible)return null;if(!modalContent)throw new Error("Modal found null modal content");var autoClose=(_c=modalContent.autoClose,_c===void 0?true:_c),body=modalContent.body,buttons=modalContent.buttons,danger=modalContent.danger,isNotDismissible=modalContent.isNotDismissible,success=modalContent.success,title=modalContent.title,warning=modalContent.warning,withCancelButton=modalContent.withCancelButton,withNoActions=modalContent.withNoActions,onClose=modalContent.onClose;var handleClose=function(){closeModal();setIsProcessingTo(false);if(onClose)onClose();};var setProcessingState=function(){return setIsProcessingTo(true);};var footerItems=function(){return buttons?createButtons(buttons,handleClose,disableActions,autoClose,setProcessingState):jsxRuntime.jsx(FooterItem$2,{children:jsxRuntime.jsx(Button,__assign({onClick:handleClose,primary:true,disabled:disableActions,success:success,danger:danger,warning:warning},{children:"OK"}))});};return jsxRuntime.jsxs(Modal,__assign({onClose:isNotDismissible?function(){}:handleClose,className:className,isAnimated:true,backdropProps:{style:{backdropFilter:blurBackdrop?"blur(4px)":undefined,fontFamily:"inherit",zIndex:theme.styles.modal.zIndex}}},{children:[title?jsxRuntime.jsx(Header,__assign({color:"",danger:danger,success:success,warning:warning},{children:title})):null,jsxRuntime.jsx(Body,{children:body}),withNoActions?null:jsxRuntime.jsxs(Footer$2,{children:[withCancelButton?jsxRuntime.jsx(FooterItem$2,{children:jsxRuntime.jsx(Button,__assign({onClick:handleClose,flat:true,loading:isProcessing,success:success,danger:danger,warning:warning},{children:"Cancel"}))}):null,footerItems()]}),isNotDismissible?null:jsxRuntime.jsx(CloseButtonWrap,{children:jsxRuntime.jsx(Button,{"aria-label":"Close modal",compact:true,danger:danger,flat:true,icon:js.mdiClose,iconSize:FONT_SIZES.MD,onClick:handleClose,primary:false,size:BUTTON_SIZES.X_SMALL,success:success,warning:warning})})]}));};var Body=styled__default["default"](Body$2)(templateObject_2||(templateObject_2=__makeTemplateObject(["\n  font-size: inherit;\n"],["\n  font-size: inherit;\n"])));var CloseButtonWrap=styled__default["default"].div(templateObject_3||(templateObject_3=__makeTemplateObject(["\n  position: absolute;\n  right: 2.3rem;\n  top: 1.3rem;\n"],["\n  position: absolute;\n  right: 2.3rem;\n  top: 1.3rem;\n"])));exports.Modal=styled__default["default"](exports.Modal)(templateObject_6||(templateObject_6=__makeTemplateObject(["\n  &&&& {\n    margin-left: ",";\n    margin-right: ",";\n    color: ",";\n    border-radius: ",";\n    max-width: calc(100vw - "," * 2);\n    width: ",";\n    z-index: ",";\n\n    ","\n\n    ","\n  }\n"],["\n  &&&& {\n    margin-left: ",";\n    margin-right: ",";\n    color: ",";\n    border-radius: ",";\n    max-width: calc(100vw - "," * 2);\n    width: ",";\n    z-index: ",";\n\n    ","\n\n    ","\n  }\n"])),SPACINGS.SM,SPACINGS.SM,function(_a){var theme=_a.theme;return theme.styles.textColorPrimary;},function(_a){var theme=_a.theme;return theme.styles.modal.borderRadius;},SPACINGS.SM,function(_a){var _b;var modalContent=_a.modalContent,theme=_a.theme;return theme.styles.modal.sizes[(_b=modalContent===null||modalContent===void 0?void 0:modalContent.size)!==null&&_b!==void 0?_b:MODAL_SIZES.SM];},function(_a){var theme=_a.theme;return theme.styles.modal.zIndex;},function(_a){var modalContent=_a.modalContent;return(modalContent===null||modalContent===void 0?void 0:modalContent.size)===MODAL_SIZES.FULL_SCREEN?styled.css(templateObject_4||(templateObject_4=__makeTemplateObject(["\n            height: calc(100vh - ("," * 2));\n            max-height: calc(100vh - ("," * 2));\n            margin: ",";\n            width: calc(100vw - ("," * 2));\n          "],["\n            height: calc(100vh - ("," * 2));\n            max-height: calc(100vh - ("," * 2));\n            margin: ",";\n            width: calc(100vw - ("," * 2));\n          "])),SPACINGS.SM,SPACINGS.SM,SPACINGS.SM,SPACINGS.SM):mediaQueries().forPhones(styled.css(templateObject_5||(templateObject_5=__makeTemplateObject(["\n            width: calc(100% - ("," * 2));\n          "],["\n            width: calc(100% - ("," * 2));\n          "])),SPACINGS.SM));},function(_a){var _css=_a._css;return _css;});var templateObject_1$2,templateObject_2,templateObject_3,templateObject_4,templateObject_5,templateObject_6;var defaultUiState={setModalState:function(){throw new Error("initial state not set");},modalState:{isModalOpen:false,modalContentState:{}}};var ModalStateContext=React.createContext(defaultUiState);var useModalState=function(){var _a=React.useContext(ModalStateContext),modalState=_a.modalState,setModalState=_a.setModalState;var closeModal=React.useCallback(function(){setModalState(function(previous){return __assign(__assign({},previous),{isModalOpen:false,modalContent:undefined,modalContentState:{}});});},[setModalState]);var openModalWith=React.useCallback(function(c){// The following check causes the app to make infinite requests due
...
//       closeModal()
//     }
setModalState(function(previous){return __assign(__assign({},previous),{isModalOpen:true,modalContent:__assign({autoClose:true},c),modalContentState:{}});});},[setModalState]);var openErrorModalWith=React.useCallback(function(c){var _a;openModalWith(__assign(__assign({},c),{title:(_a=c.title)!==null&&_a!==void 0?_a:"Uh Oh!",danger:true}));},[openModalWith]);var openSuccessModalWith=React.useCallback(function(c){var _a;openModalWith(__assign(__assign({},c),{title:(_a=c.title)!==null&&_a!==void 0?_a:"Success!",success:true}));},[openModalWith]);var openWarningModalWith=React.useCallback(function(c){var _a;openModalWith(__assign(__assign({},c),{title:(_a=c.title)!==null&&_a!==void 0?_a:"Warning!",warning:true}));},[openModalWith]);var setModalContentState=React.useCallback(function(state){setModalState(function(previous){return __assign(__assign({},previous),{modalContentState:__assign(__assign({},previous.modalContentState),state)});});},[setModalState]);return{closeModal:closeModal,openErrorModalWith:openErrorModalWith,openModalWith:openModalWith,openSuccessModalWith:openSuccessModalWith,openWarningModalWith:openWarningModalWith,setModalContentState:setModalContentState,setModalState:setModalState,modalState:modalState};};var ModalManager=function(_a){var blurBackdrop=_a.blurBackdrop,_css=_a._css;var _b=useModalState(),closeModal=_b.closeModal,modalState=_b.modalState;var isModalOpen=modalState.isModalOpen,modalContent=modalState.modalContent;return jsxRuntime.jsx(exports.Modal,{blurBackdrop:blurBackdrop,closeModal:closeModal,_css:styled.css(templateObject_1$1||(templateObject_1$1=__makeTemplateObject(["\n        max-width: 90%;\n        ","\n      "],["\n        max-width: 90%;\n        ","\n      "])),_css!==null&&_css!==void 0?_css:""),disableActions:false,isVisible:isModalOpen,modalContent:modalContent});};var templateObject_1$1;var ModalStateProvider=function(_a){var children=_a.children;var defaultState=React.useContext(ModalStateContext);var _b=__read(React.useState(defaultState.modalState),2),state=_b[0],setState=_b[1];var _c=__read(React.useState({setModalState:setState,modalState:state}),2),modalStateWithDispatch=_c[0],setModalStateWithDispatch=_c[1];React.useEffect(function(){setModalStateWithDispatch({setModalState:setState,modalState:state});},[state]);return jsxRuntime.jsx(ModalStateContext.Provider,__assign({value:modalStateWithDispatch},{children:children}));};var DrawerModal=styled__default["default"](function(_a){var className=_a.className,drawerModalBody=_a.drawerModalBody,footerActions=_a.footerActions,_b=_a.isOpen,isOpen=_b===void 0?false:_b,onCloseDrawerModal=_a.onCloseDrawerModal,title=_a.title;return jsxRuntime.jsxs(DrawerModal$1,__assign({backdropProps:{style:{fontFamily:"inherit"}},className:className,isOpen:isOpen,onClose:onCloseDrawerModal},{children:[title?jsxRuntime.jsx(DrawerModal$1.Header,__assign({tag:"h2"},{children:title})):null,jsxRuntime.jsx(DrawerModal$1.Body,{children:drawerModalBody}),footerActions?jsxRuntime.jsx(DrawerModal$1.Footer,{children:footerActions.map(function(n,i){return jsxRuntime.jsx(DrawerModal$1.FooterItem,{children:n},i);})}):null,title?jsxRuntime.jsx(DrawerModal$1.Close,{}):null]}));})(templateObject_1||(templateObject_1=__makeTemplateObject(["\n  font-family: inherit;\n  width: ",";\n  max-width: calc(100vw - ",");\n  ","\n"],["\n  font-family: inherit;\n  width: ",";\n  max-width: calc(100vw - ",");\n  ","\n"])),function(_a){var width=_a.width,theme=_a.theme;return width!==null&&width!==void 0?width:theme.styles.drawer.width;},SPACINGS.SM,function(_a){var _css=_a._css;return _css;});var templateObject_1;exports.DrawerModal=DrawerModal;exports.MODAL_SIZES=MODAL_SIZES;exports.ModalManager=ModalManager;exports.ModalStateContext=ModalStateContext;exports.ModalStateProvider=ModalStateProvider;exports.defaultUiState=defaultUiState;exports.useModalState=useModalState;
