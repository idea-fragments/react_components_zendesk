...
typeof options.getShadowRoot==='function'&&options.getShadowRoot(element);if(shadowRoot){// add shadow dom scope IIF a shadow root node was given; otherwise, an undisclosed
...
isDetailsWithSummary(node)||isDisabledFromFieldset(node)){return false;}return true;};var isNodeMatchingSelectorTabbable=function isNodeMatchingSelectorTabbable(options,node){if(isNonTabbableRadio(node)||getTabindex(node)<0||!isNodeMatchingSelectorFocusable(options,node)){return false;}return true;};/**
 * @param {Array.<Element|CandidatesScope>} candidates
 * @returns Element[]
 */var sortByOrder=function sortByOrder(candidates){var regularTabbables=[];var orderedTabbables=[];candidates.forEach(function(item,i){var isScope=!!item.scope;var element=isScope?item.scope:item;var candidateTabindex=getTabindex(element,isScope);var elements=isScope?sortByOrder(item.candidates):element;if(candidateTabindex===0){isScope?regularTabbables.push.apply(regularTabbables,elements):regularTabbables.push(element);}else{orderedTabbables.push({documentOrder:i,tabIndex:candidateTabindex,item:item,isScope:isScope,content:elements});}});return orderedTabbables.sort(sortOrderedTabbables).reduce(function(acc,sortable){sortable.isScope?acc.push.apply(acc,sortable.content):acc.push(sortable.content);return acc;},[]).concat(regularTabbables);};var tabbable=function tabbable(el,options){options=options||{};var candidates;if(options.getShadowRoot){candidates=getCandidatesIteratively([el],options.includeContainer,{filter:isNodeMatchingSelectorTabbable.bind(null,options),flatten:false,getShadowRoot:options.getShadowRoot});}else{candidates=getCandidates(el,options.includeContainer,isNodeMatchingSelectorTabbable.bind(null,options));}return sortByOrder(candidates);};/**
 * Copyright Zendesk, Inc.
 *
 * Use of this source code is governed under the Apache License, Version 2.0
 * found at http://www.apache.org/licenses/LICENSE-2.0.
 */function _extends$8(){_extends$8=Object.assign||function(target){for(var i=1;i<arguments.length;i++){var source=arguments[i];for(var key in source){if(Object.prototype.hasOwnProperty.call(source,key)){target[key]=source[key];}}}return target;};return _extends$8.apply(this,arguments);}function _objectWithoutPropertiesLoose$7(source,excluded){if(source==null)return{};var target={};var sourceKeys=Object.keys(source);var key,i;for(i=0;i<sourceKeys.length;i++){key=sourceKeys[i];if(excluded.indexOf(key)>=0)continue;target[key]=source[key];}return target;}function ownerDocument$1(node){return node&&node.ownerDocument||document;}function activeElement(doc){if(doc===void 0){doc=ownerDocument$1();}try{var active=doc.activeElement;if(!active||!active.nodeName)return null;return active;}catch(e){return doc.body;}}var _excluded$1$6=["onKeyDown"];var useFocusJail=function useFocusJail(_temp){var _ref=_temp===void 0?{containerRef:React.createRef()}:_temp,_ref$focusOnMount=_ref.focusOnMount,focusOnMount=_ref$focusOnMount===void 0?true:_ref$focusOnMount,_ref$restoreFocus=_ref.restoreFocus,restoreFocus=_ref$restoreFocus===void 0?true:_ref$restoreFocus,environment=_ref.environment,focusElem=_ref.focusElem,containerRef=_ref.containerRef;var restoreFocusElement=React.useRef(null);var _useState=React.useState(containerRef.current),currentRef=_useState[0],setCurrentRef=_useState[1];React.useEffect(function(){if(containerRef.current!==currentRef){setCurrentRef(containerRef.current);}});var focusElement=React.useCallback(function(element){if(focusElem){focusElem(element);}else{element&&element.focus();}},[focusElem]);var validateContainerRef=function validateContainerRef(){if(!currentRef){throw new Error('Accessibility Error: You must apply the ref prop to your containing element.');}};var getInitialFocusNode=function getInitialFocusNode(){var doc=environment?environment:document;var activeElem=activeElement(doc);var containerElem=currentRef;return containerElem.contains(activeElem)?activeElem:containerElem;};var getTabbableNodes=function getTabbableNodes(){var elements=tabbable(currentRef);return{firstItem:elements[0]||getInitialFocusNode(),lastItem:elements[elements.length-1]||getInitialFocusNode()};};var getContainerProps=function getContainerProps(_temp2){var _ref2=_temp2===void 0?{}:_temp2,onKeyDown=_ref2.onKeyDown,other=_objectWithoutPropertiesLoose$7(_ref2,_excluded$1$6);return _extends$8({onKeyDown:composeEventHandlers(onKeyDown,function(event){if(event.keyCode!==KEY_CODES.TAB){return;}validateContainerRef();var tabbableNodes=getTabbableNodes();if(event.shiftKey&&(event.target===tabbableNodes.firstItem||event.target===currentRef)){focusElement(tabbableNodes.lastItem);event.preventDefault();}if(!event.shiftKey&&event.target===tabbableNodes.lastItem){focusElement(tabbableNodes.firstItem);event.preventDefault();}}),'data-garden-container-id':'containers.focusjail','data-garden-container-version':'1.4.13'},other);};React.useEffect(function(){restoreFocusElement.current=activeElement(environment||document);if(focusOnMount){focusElement(currentRef);}return function(){var isBodyInactive=restoreFocusElement.current!==document.body;var hasActiveElement=restoreFocusElement.current!==null;if(isBodyInactive&&hasActiveElement&&restoreFocus){focusElement(restoreFocusElement.current);}};},[focusOnMount,restoreFocus,environment,focusElement,currentRef]);return{getContainerProps:getContainerProps,focusElement:focusElement};};({children:propTypes.exports.func,render:propTypes.exports.func,focusOnMount:propTypes.exports.bool,restoreFocus:propTypes.exports.bool,environment:propTypes.exports.any,containerRef:propTypes.exports.any.isRequired,focusElem:propTypes.exports.func});/**
 * Copyright Zendesk, Inc.
 *
 * Use of this source code is governed under the Apache License, Version 2.0
 * found at http://www.apache.org/licenses/LICENSE-2.0.
 */function _extends$7(){_extends$7=Object.assign||function(target){for(var i=1;i<arguments.length;i++){var source=arguments[i];for(var key in source){if(Object.prototype.hasOwnProperty.call(source,key)){target[key]=source[key];}}}return target;};return _extends$7.apply(this,arguments);}function _objectWithoutPropertiesLoose$6(source,excluded){if(source==null)return{};var target={};var sourceKeys=Object.keys(source);var key,i;for(i=0;i<sourceKeys.length;i++){key=sourceKeys[i];if(excluded.indexOf(key)>=0)continue;target[key]=source[key];}return target;}var _excluded$1$5=["onMouseUp"],_excluded2$2=["role","onKeyDown","onMouseDown"],_excluded3=["id"],_excluded4=["id"],_excluded5=["onClick"];function useModal(_temp){var _ref=_temp===void 0?{}:_temp,onClose=_ref.onClose,modalRef=_ref.modalRef,_id=_ref.id,focusOnMount=_ref.focusOnMount,restoreFocus=_ref.restoreFocus,environment=_ref.environment;var seed=useUIDSeed();var _useState=React.useState(_id||seed("modal_"+'0.8.15')),idPrefix=_useState[0];var titleId=idPrefix+"--title";var contentId=idPrefix+"--content";var isModalMousedDownRef=React.useRef(false);var closeModal=function closeModal(event){onClose&&onClose(event);};var getBackdropProps=function getBackdropProps(_temp2){var _ref2=_temp2===void 0?{}:_temp2,onMouseUp=_ref2.onMouseUp,other=_objectWithoutPropertiesLoose$6(_ref2,_excluded$1$5);var containerId='containers.modal';return _extends$7({onMouseUp:composeEventHandlers(onMouseUp,function(event){var target=event.target;var isModalContainer=containerId===target.getAttribute('data-garden-container-id');if(!isModalMousedDownRef.current&&isModalContainer){closeModal(event);}isModalMousedDownRef.current=false;}),'data-garden-container-id':containerId,'data-garden-container-version':'0.8.15'},other);};var _getModalProps=function getModalProps(_temp3){var _ref3=_temp3===void 0?{}:_temp3,_ref3$role=_ref3.role,role=_ref3$role===void 0?'dialog':_ref3$role,onKeyDown=_ref3.onKeyDown,onMouseDown=_ref3.onMouseDown,other=_objectWithoutPropertiesLoose$6(_ref3,_excluded2$2);return _extends$7({role:role,tabIndex:-1,'aria-modal':true,'aria-labelledby':titleId,'aria-describedby':contentId,onMouseDown:composeEventHandlers(onMouseDown,function(){isModalMousedDownRef.current=true;}),onKeyDown:composeEventHandlers(onKeyDown,function(event){if(event.keyCode===KEY_CODES.ESCAPE){closeModal(event);}})},other);};var getTitleProps=function getTitleProps(_temp4){var _ref4=_temp4===void 0?{}:_temp4,_ref4$id=_ref4.id,id=_ref4$id===void 0?titleId:_ref4$id,other=_objectWithoutPropertiesLoose$6(_ref4,_excluded3);return _extends$7({id:id},other);};var getContentProps=function getContentProps(_temp5){var _ref5=_temp5===void 0?{}:_temp5,_ref5$id=_ref5.id,id=_ref5$id===void 0?contentId:_ref5$id,other=_objectWithoutPropertiesLoose$6(_ref5,_excluded4);return _extends$7({id:id},other);};var getCloseProps=function getCloseProps(_temp6){var _ref6=_temp6===void 0?{}:_temp6,onClick=_ref6.onClick,other=_objectWithoutPropertiesLoose$6(_ref6,_excluded5);return _extends$7({'aria-label':'Close modal',onClick:composeEventHandlers(onClick,function(event){closeModal(event);})},other);};var _useFocusJail=useFocusJail({containerRef:modalRef,focusOnMount:focusOnMount,restoreFocus:restoreFocus,environment:environment}),getContainerProps=_useFocusJail.getContainerProps;return{getBackdropProps:getBackdropProps,getModalProps:function getModalProps(props){return getContainerProps(_getModalProps(props));},getTitleProps:getTitleProps,getContentProps:getContentProps,getCloseProps:getCloseProps,closeModal:closeModal};}({children:propTypes.exports.func,render:propTypes.exports.func,onClose:propTypes.exports.func,modalRef:propTypes.exports.any.isRequired,id:propTypes.exports.string,focusOnMount:propTypes.exports.bool,restoreFocus:propTypes.exports.bool,environment:propTypes.exports.any});function mergeRefs(refs){return function(value){refs.forEach(function(ref){if(typeof ref==="function"){ref(value);}else if(ref!=null){ref.current=value;}});};}React__namespace.createContext();React__namespace.createContext();/**
 ...
 */process.env.NODE_ENV!=='production';function _objectWithoutPropertiesLoose$5(source,excluded){if(source==null)return{};var target={};var sourceKeys=Object.keys(source);var key,i;for(i=0;i<sourceKeys.length;i++){key=sourceKeys[i];if(excluded.indexOf(key)>=0)continue;target[key]=source[key];}return target;}/**
...
if(phase==='active'){/* eslint-disable no-unused-expressions */node&&node.scrollTop;}if(className){this.appliedClasses[type][phase]=className;_addClass(node,className);}};_proto.removeClasses=function removeClasses(node,type){var _this$appliedClasses$=this.appliedClasses[type],baseClassName=_this$appliedClasses$.base,activeClassName=_this$appliedClasses$.active,doneClassName=_this$appliedClasses$.done;this.appliedClasses[type]={};if(baseClassName){removeClass(node,baseClassName);}if(activeClassName){removeClass(node,activeClassName);}if(doneClassName){removeClass(node,doneClassName);}};_proto.render=function render(){var _this$props=this.props;_this$props.classNames;var props=_objectWithoutPropertiesLoose$5(_this$props,["classNames"]);return/*#__PURE__*/React__default["default"].createElement(Transition,_extends$9({},props,{onEnter:this.onEnter,onEntered:this.onEntered,onEntering:this.onEntering,onExit:this.onExit,onExiting:this.onExiting,onExited:this.onExited}));};return CSSTransition;}(React__default["default"].Component);CSSTransition.defaultProps={classNames:''};CSSTransition.propTypes=process.env.NODE_ENV!=="production"?_extends$9({},Transition.propTypes,{/**
...
var isArray=function(o){return o instanceof Array;};var COMPONENT_NAME="Icon";var Icon=function(_a){var color=_a.color,_b=_a.size,size=_b===void 0?1:_b,svg=_a.svg,title=_a.title,_c=_a.onClick,onClick=_c===void 0?DO_NOTHING:_c;return isArray(svg)// @ts-ignore
?jsxRuntime.jsx(MIcon.Stack,__assign({onClick:onClick,size:size,title:title,color:color||"currentColor","data-component-name":COMPONENT_NAME},{children:svg.map(function(s,i){return jsxRuntime.jsx(MIcon__default["default"],{path:s},i);})})):jsxRuntime.jsx(MIcon__default["default"],{path:svg,// @ts-ignore
onClick:onClick,size:size,title:title,color:color||"currentColor","data-component-name":COMPONENT_NAME});};Icon.COMPONENT_NAME=COMPONENT_NAME;var XS$1=".5rem";var SM$1="1rem";var MD$1="1.5rem";var LG$1="2rem";var XL$1="2.5rem";var XXL$1="3rem";var XXXL$1="4rem";var XXXXL$1="5rem";var SPACINGS=Object.freeze({XS:XS$1,SM:SM$1,MD:MD$1,LG:LG$1,XL:XL$1,XXL:XXL$1,XXXL:XXXL$1,XXXXL:XXXXL$1});Object.freeze({DOWN:"down",RIGHT:"right"});var getFlexDirection=function(_a){var withRows=_a.withRows;return withRows?"column":"row";};var FlexBox=styled__default["default"].div(templateObject_1$2||(templateObject_1$2=__makeTemplateObject(["\n  align-items: ",";\n  display: ",";\n  flex: ",";\n  flex-direction: ",";\n  gap: ",";\n  justify-content: ",";\n  ","\n\n  ","\n"],["\n  align-items: ",";\n  display: ",";\n  flex: ",";\n  flex-direction: ",";\n  gap: ",";\n  justify-content: ",";\n  ","\n\n  ","\n"])),function(_a){var alignItems=_a.alignItems;return alignItems?alignItems:"initial";},function(p){return p.inline?"inline-flex":"flex";},function(_a){var fluid=_a.fluid;return fluid?1:"unset";},getFlexDirection,function(p){return p.gap?p.gap:SPACINGS.SM;},function(p){return p.justifyContent||"initial";},function(p){return p.wrap?wrappedStyling:"";},function(p){return p.responsivePropsList?responsiveStyles(p):"";});FlexBox.defaultProps={withRows:false};var responsiveStyles=function(_a){var responsivePropsList=_a.responsivePropsList,originalProps=__rest(_a,["responsivePropsList"]);return responsivePropsList===null||responsivePropsList===void 0?void 0:responsivePropsList.map(function(_a){var mediaQueryFunc=_a.mediaQueryFunc,rProps=_a.props;var props=__assign(__assign({},originalProps),rProps);var alignItems=props.alignItems,gap=props.gap,inline=props.inline,justifyContent=props.justifyContent,fluid=props.fluid,wrap=props.wrap;return mediaQueryFunc(styled.css(templateObject_2$2||(templateObject_2$2=__makeTemplateObject(["\n      align-items: ",";\n      display: ",";\n      flex: ",";\n      flex-direction: ",";\n      gap: ",";\n      justify-content: ",";\n      ","\n    "],["\n      align-items: ",";\n      display: ",";\n      flex: ",";\n      flex-direction: ",";\n      gap: ",";\n      justify-content: ",";\n      ","\n    "])),alignItems?alignItems:"initial",inline?"inline-flex":"flex",fluid?1:"unset",getFlexDirection(props),gap?gap:SPACINGS.SM,justifyContent||"initial",wrap?wrappedStyling:"")).join("");});};var wrappedStyling=styled.css(templateObject_3$2||(templateObject_3$2=__makeTemplateObject(["\n  flex-wrap: wrap;\n"],["\n  flex-wrap: wrap;\n"])));var templateObject_1$2,templateObject_2$2,templateObject_3$2;/**
 * Copyright Zendesk, Inc.
 *
 * Use of this source code is governed under the Apache License, Version 2.0
 * found at http://www.apache.org/licenses/LICENSE-2.0.
