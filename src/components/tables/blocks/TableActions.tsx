...
export type TableActionsProps = { compact?: boolean } &
                                Pick<TableProps, "columnConfigs"> &
                                Partial<Pick<TableProps,
                                  "actions" |
                                  "filterState" |
                                  "onColumnSort" |
                                  "onFiltersChange" |
...
export const TableActions: FC<TableActionsProps> = ({
  ...
  const submitFilters = useCallback((fs: FilterState) => {
    onFiltersChange!(fs)
    closeModal()
  }, [closeModal, onFiltersChange])

  const submitSort = useCallback((s: SortState) => {
    onColumnSort!(s)
    closeModal()
  ...
  const openFiltersModal = useCallback(() => {
    openModalWith({
      body:          <TableFilters columnConfigs={columnConfigs}
  ...
  const openSorterModal = useCallback(() => {
    openModalWith({
      body:          <TableSorter columnConfigs={columnConfigs}
  ...
  const defaultTableActions = useMemo((): TableAction[] => {
    ...
    if (onFiltersChange) list.push({
      label:       "Filter",
      onClick:     openFiltersModal,
      buttonProps: { primary: false }
    })
    if (onColumnSort) list.push({
      label:       "Sort",
  ...
  let [compactableActions, nonCompactableActions] = useMemo(
    (): [TableAction[], TableAction[]] => {
      ...
      const groupedActions = list.reduce((partitions, action) => {
        const [compactables, nonCompactables] = partitions
        const compactable                     = !action.notCompactable

        compact && compactable ? compactables.push(action) : nonCompactables.push(action)
        return partitions
