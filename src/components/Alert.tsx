...
export let Alert: FC<Props> = ({
  ...
  return (
    <AbsoluteContainer>
      <AlertWrapper className={className} color={bgColor}>
        <FlexBox alignItems={"center"}
                 css={`
                   border-right: 1px solid ${dark(bgColor, .15)};
                   padding: 0 ${SM} 0 ${SM};
                 `}>
        ...
        <Content>
          {title
           ? <Text css={`font-weight: ${FONT_WEIGHTS.MEDIUM};`}>{title}</Text>
           : null}
          <Text>{body}</Text>
        </Content>
        <FlexBox alignItems={"center"}
                 css={`padding: 0 ${SM} 0 ${SM};`}>
