...
export type SliderProps = FormFieldProps & ColorProps & {
  min?: number,
  max?: number,
...
export let Slider: FC<SliderProps> = ({
  ...
  useEffect(() => { syncValueFromParent() }, [syncValueFromParent])
  ...
  return <Container fluid={fluid} gap={null} withRows>
    <Field>
      ...
      {
        hint
        ? <Hint _css={css`&&& {font-size: ${FONT_SIZES.XS};}`}>{hint}</Hint>
        : null
      }
...
const disabledThumbStyles = css<{ disabled?: boolean }>`
  ${({ disabled }) => disabled ? `background-color: ${COLORS.LIGHT_GREY}` : ""};
`
...
const Range = styled(ZRange).attrs(({ color, theme }: StyledProps<{ color?: string }>) => ({
