...
export const ImageCropper: FC<ImageCropperProps> = ({
  ...
  const canvasToFile = useCallback(async (canvas: HTMLCanvasElement): Promise<File> => {
    return new Promise((resolve) => {
      const toFile = (blob: Nullable<Blob>) => {
        ...
        resolve(new File([blob], imageData.file?.name ?? `${crypto.randomUUID()}.jpg`))
      }
  ...
  const canvasToDataUrl = useCallback((canvas: HTMLCanvasElement): string => {
    return canvas.toDataURL("image/jpeg", imageQuality)
  }, [imageQuality])

  const croppedImageData = useCallback(async (): Promise<Nullable<ImageData>> => {
    ...
    return { dataURL: canvasToDataUrl(canvas), file: await canvasToFile(canvas) }
  ...
  return <FullScreen withRows>
    <Container>
      <Cropper
        ...
        setImageRef={(ref) => {imageRef.current = ref.current ?? undefined}}
        zoom={zoom}
      />
    </Container>
