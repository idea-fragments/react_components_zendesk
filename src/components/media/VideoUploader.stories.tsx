...
import {
  VideoData,
  VideoUploader
}                             from "components/media/VideoUploader"
  ...
  return <div>
    {
      ...
      ? <FlexBox _css={`height: 100px; width: 100px;`}>
        {
          args.shouldCrop
          ? <CroppedVideoPlayer onLoad={() => { console.log("video loaded")}} videoData={video} />
          : <VideoPlayer videoData={video} />
        }
      </FlexBox>
      : null
    }
    <VideoUploader {...args} onChange={setVideo} videoData={video} />
  </div>
}
