    ...
    const createActionButtons = (actions: NavigationAction[]) =>
      actions.map(({ as, ...a }: NavigationAction) => {
        const props = {
          ...
          compact: true,
          _css:
            !a.icon && fallbackToText ? textButtonStyles : actionButtonStyles,
          icon: a.icon,
          iconPosition: "left",
...
const actionButtonStyles = css<{ active: boolean }>`
  &&&&& {
    border-radius: ${({ theme }) =>
      `calc(${theme.styles.sidebar.actionButton.borderRadius} + 10px)`};
    ${({ active, theme }) =>
      !active
        ? `background: transparent; color: ${fade(
...
const BarWrapper = styled.header<StyledProps<{ bordered: boolean }>>`
  ...
  position: fixed;
  ${({ theme }) =>
    theme.styles.appBar.screenPosition === "top" ? "top: 0;" : "bottom: 0;"}
  left: 0;
  width: 100%;
