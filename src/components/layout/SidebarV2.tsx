...
const actionButtonStyles = css<{ active: boolean }>`
  &&&&& {
    border-radius: ${({ theme }) =>
      theme.styles.sidebar.actionButton.borderRadius};
    justify-content: flex-start;
    background: ${({ active, theme }) =>
      active ? theme.styles.sidebar.actionButton.color : "transparent"};
    color: ${({ active, theme }) => {
      return active
        ? theme.styles.textColorPrimary
        : fade(theme.styles.textColorPrimary, 0.2)
    }};
