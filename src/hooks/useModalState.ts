...
export const useModalState = (): ModalStateWithDispatch & ModalHelperFunctions => {
  ...
  const openModalWith = useCallback((c: ModalContent) => {
  ...
  const openErrorModalWith = useCallback((c: ModalContent) => {
    openModalWith({ ...c, title: c.title ?? "Uh Oh!", danger: true, })
  }, [openModalWith])

  const openSuccessModalWith = useCallback((c: ModalContent) => {
    openModalWith({ ...c, title: c.title ?? "Success!", success: true, })
  }, [openModalWith])

  const openWarningModalWith = useCallback((c: ModalContent) => {
    openModalWith({ ...c, title: c.title ?? "Warning!", warning: true, })
  }, [openModalWith])

  const setModalContentState = useCallback((state: Record<string, any>) => {
    setModalState((previous: ModalState) => ({
      ...previous,
